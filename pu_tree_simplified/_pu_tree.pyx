...
cdef class TreeBuilder:
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
                np.ndarray sample_weight=None, np.ndarray p_y=None,
                 np.ndarray U=None, np.ndarray N=None):
        """Build a decision tree from the training set (X, y)."""
        pass

    cdef inline _check_input(self, object X, np.ndarray y,
                             np.ndarray sample_weight, np.ndarray p_y):
        ...
        if (p_y is not None and
            (p_y.dtype != DOUBLE or
            not p_y.flags.contiguous)):
                p_y = np.asarray(p_y, dtype=DOUBLE, order="C")


        return X, y, sample_weight, p_y
...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(self, Tree tree, object X, np.ndarray y,
        ...
        # check input
        X, y, sample_weight, p_y = self._check_input(X, y, sample_weight, p_y)

        cdef DOUBLE_t* sample_weight_ptr = NULL
        cdef DOUBLE_t* p_y_ptr = NULL
        cdef DOUBLE_t* U_ptr = NULL
        cdef DOUBLE_t* N_ptr = NULL
        
        if sample_weight is not None:
            sample_weight_ptr = <DOUBLE_t*> sample_weight.data
            
        if p_y is not None:
            p_y_ptr = <DOUBLE_t*> p_y.data

        if U is not None:
            U_ptr = <DOUBLE_t*> U.data
            
        if N is not None:
            N_ptr = <DOUBLE_t*> N.data
        ...
        splitter.init(X, y, sample_weight_ptr, p_y_ptr, U_ptr, N_ptr)
