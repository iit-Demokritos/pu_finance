...
class ElkanotoPuClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, estimator, hold_out_ratio=0.1, random_state=42):
    ...
    def fit(self, X, y):
        ...
        np.random.default_rng(seed=self.random_state).shuffle(all_indices)
    ...
    def predict_proba(self, X):
        ...
        proba = probabilistic_predictions[:, 1] / self.c
    ...
    def predict(self, X, treshold=0.5):
        ...
        return (probas[:, 1] > treshold).astype(int)
...
class WeightedElkanotoPuClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        self, estimator, labeled=-1, unlabeled=-1, hold_out_ratio=0.1, random_state=42
    ):
        self.estimator = estimator
        self.c = 1.0
        self.hold_out_ratio = hold_out_ratio
    ...
    def fit(self, X, y):
        ...
        if self.labeled == -1:
            self.labeled = int(positives.shape[0] * (1 - self.hold_out_ratio))
            self.unlabeled = int(positives.shape[0] * self.hold_out_ratio)
